...
struct prefix_master {
	...
	/* value to return if a prefix_list is empty and we are trying to get a match */
...
static struct prefix_list *prefix_list_lookup_do(afi_t afi, int orf, int rtc, const char *name)
{
...
static struct prefix_list *prefix_list_insert(afi_t afi, int orf, int rtc, const char *name)
{
...
int prefix_bgp_rtc_set(char *name, struct prefix *p, int permit, int set)
{
	...
	/* request route-target match only in prefix_match() */
	SET_FLAG(np.u.prefix_rtc.flags, RTC_INFO_FLAG_RT_MATCH);
