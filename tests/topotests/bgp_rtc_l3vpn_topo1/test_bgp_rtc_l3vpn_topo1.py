...
def test_as_path_change_step1():
    ...
    for rname, router in tgen.routers().items():
        ...
        reffile = os.path.join(CWD, f"{rname}/show_bgp_ipv4_rtc_step1.json")
        expected = json.loads(open(reffile).read())
        cmd = "show bgp ipv4 rt-constraint json"
        test_func = functools.partial(
            topotest.router_json_cmp,
    ...
    for rname, router in tgen.routers().items():
        for ipv in [4, 6]:
            safi = "unicast" if "ce" in rname else "vpn"
            reffile = os.path.join(CWD, f"{rname}/show_bgp_ipv{ipv}_{safi}_step1.json")
            expected = json.loads(open(reffile).read())
            exact = not expected  # exact match if json is void (ie. {})
            cmd = f"show bgp ipv{ipv} {safi} json"
            test_func = functools.partial(
