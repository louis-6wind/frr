...
def setup_module(mod):
    ...
    for r in range(1, 5):
        ...
        router.cmd(
            f"ip link add vxlan{i} type vxlan id {i} dstport 4789 local 10.0.1.{r}0 nolearning"
        )
        router.cmd(f"brctl addbr br{i}")
        router.cmd(f"brctl addif br{i} vxlan{i}")
        router.cmd(f"brctl addif br{i} eth-r{r}")
        router.cmd(f"brctl stp br{i} off")
        router.cmd(f"ip link set vxlan{i} up")
        router.cmd(f"ip link set br{i} up")
